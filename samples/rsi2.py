...
def cci(ohlc, period = 2):
    ...
    dn[dn>0] = 0
    dn = -dn
    #ohlc['CCI'] = 100*pd.rolling_mean(up, period)/(pd.rolling_mean(up, period)+pd.rolling_mean(dn, period))
    ohlc['CCI'] = 100*pd.rolling_mean(dn, period)/(pd.rolling_mean(up, period)+pd.rolling_mean(dn, period))
    return ohlc
